...
class CustomVerticalListviewList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Obx(
      () => isLoading.value
          ? _buildShimmerList()
          : SizedBox(
              height: items.length * (isFromCartScreen ? 200.h : 182.h),
          ...
          children: [
            Flexible(
              child: Row(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _buildImageSection(item, index, isFromCartScreen),
                  Expanded(child: _buildContentSection(item)),
                ],
              ),
            ),
            if (isFromCartScreen)
  ...
  Widget _buildImageSection(ServiceItem item, int index, bool isFromCart) {
    return Stack(
  ...
  Map<String, dynamic> _getStaticDetailData(ServiceItem item) {
    return {
      ...
      'status':
          'bidding', // Can be 'bidding', 'waiting', 'before_pay', 'closed'
      'mediaUrls': [
      ...
      'reviews': [
        {
          'rating': 5.0,
          'comment':
              'Excellent work! Exceeded my expectations. Will definitely work with this seller again.',
        ...
        },
        {
          'rating': 4.5,
          'comment':
              'Great quality and fast delivery. Very professional service.',
